...
def foreCast7Days(province):
    print(province)
    print("แสดงข้อมูล ผลการพยากรณ์อากาศสำหรับประเทศไทยล่วงหน้า 7 วัน ของจังหวัด" + pro)
    url = 'https://data.tmd.go.th/api/WeatherForecast7Days/V1/'
    response = requests.request('GET', url, params=querystring)
    response = json.loads(response.text)
    string = ""
    for i in response['Provinces']:
        print(i['ProvinceNameTh'])
        if i['ProvinceNameTh'] == province:
            print('true')
            break
        #     print('11111')
        #     break
